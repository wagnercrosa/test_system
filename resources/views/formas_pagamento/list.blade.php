	...
	<div class="card-body">
		<div class="@if(getenv('ANIMACAO')) animate__animated @endif animate__backInLeft">
			<div class="col-sm-12 col-lg-4 col-md-6 col-xl-4">
				...
				<a href="/formasPagamento/new" class="btn btn-lg btn-success">
					<i class="fa fa-plus"></i>Novo forma de pagamento
				</a>
			</div>
		</div>
					...
					<table class="datatable-table" style="max-width: 100%; overflow: scroll">
						<thead class="datatable-head">
							<tr class="datatable-row" style="left: 0px;">
								...
								<th data-field="OrderID" class="datatable-cell datatable-cell-sort"><span style="width: 120px;">Prazo dias</span></th>
							...
							<tr class="datatable-row">
								...
								<td class="datatable-cell">
									<span class="codigo" style="width: 120px;">
										@if($f->tipo_taxa == 'valor') R$ @endif 
										{{ number_format($f->taxa, 2, ',', '.') }}
										@if($f->tipo_taxa == 'perc') % @endif 
								...
								<td class="datatable-cell">
									<span class="codigo" style="width: 200px;">
										...
										<a href="/formasPagamento/edit/{{$f->id}}" class="btn btn-sm btn-warning">
											<i class="la la-edit"></i>
										</a>
										@if(!in_array($f->chave, $tiposNaoDelete))
										<a onclick='swal("Atenção!", "Deseja remover este registro?", "warning").then((sim) => {if(sim){ location.href="/formasPagamento/delete/{{ $f->id }}" }else{return false} })' href="#!"class="btn btn-sm btn-danger">
