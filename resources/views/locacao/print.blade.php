...
<body>
	...
	<table>
		...
		<tbody>
			...
			<tr>
				<th class="b-top">{{$l->id}}</th>
				<th class="b-top">{{$l->cliente->razao_social}} | {{$l->cliente->rua}}, {{$l->cliente->numero}} - {{$l->cliente->bairro}} - {{$l->cliente->cidade->nome}} ({{$l->cliente->cidade->uf}})</th>
			...
			@if(sizeof($l->itens) > 0)
			<tr>
				<th colspan="2" style="color: red">ITENS/PRODUTOS</th>
			</tr>

			@foreach($l->itens as $i)
			<tr>
				<th colspan="4" style="width: 400px;" class="b-top">{{$i->produto->nome}} {{$i->observacao}}</th>
				<th style="width: 80px;" class="b-top">{{$i->valor}}</th>
