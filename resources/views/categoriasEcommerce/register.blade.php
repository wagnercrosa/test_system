...
<div class=" d-flex flex-column flex-column-fluid" id="kt_content">
	<div class="card card-custom gutter-b example example-compact">
		<div class="container @if(getenv('ANIMACAO')) animate__animated @endif animate__backInLeft">
			<div class="col-lg-12">
				<br>
				<form method="post" action="{{{ isset($categoria) ? '/categoriaEcommerce/update': '/categoriaEcommerce/save' }}}" enctype="multipart/form-data">

					<input type="hidden" name="id" value="{{{ isset($categoria) ? $categoria->id : 0 }}}">
					<div class="card card-custom gutter-b example example-compact">
						<div class="card-header">

							<h3 class="card-title">{{isset($categoria) ? 'Editar' : 'Nova'}} Categoria</h3>
						</div>
										...
										<div class="form-group validated col-sm-6 col-lg-6 col-12">
											...
											<div class="">
												<span class="switch switch-outline switch-info">
													<label>
														<input value="true" @if(isset($categoria) && $categoria->destaque) checked @endif type="checkbox" name="destaque" id="destaque">
														<span></span>
													</label>
												</span>
											</div>
										...
										<div class="form-group row">
											...
											<div class="col-lg-10 col-xl-6">
												...
												<div class="image-input image-input-outline" id="kt_image_1">
													<div class="image-input-wrapper" @if(isset($categoria) && file_exists(public_path('ecommerce/categorias/').$categoria->path)) style="background-image: url(/ecommerce/categorias/{{$categoria->img}})" @else style="background-image: url(/imgs/no_image.png)" @endif></div>
													<label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="tooltip" title="" data-original-title="Change avatar">
														<i class="fa fa-pencil icon-sm text-muted"></i>
														<input value="{{old('file')}}" type="file" name="file" accept=".png, .jpg, .jpeg">
